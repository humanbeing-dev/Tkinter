...
class Application(Frame):
    ...
    def pressing(self, event):
        x = 0
        y = 0
        if event.keysym == "Left" and self.my_canvas.coords(self.my_circle)[0] > 0: x = -10
        if event.keysym == "Right" and self.my_canvas.coords(self.my_circle)[2] < self.w: x = 10
        if event.keysym == "Up" and self.my_canvas.coords(self.my_circle)[1] > 0: y = -10
        if event.keysym == "Down" and self.my_canvas.coords(self.my_circle)[3] < self.h: y = 10
